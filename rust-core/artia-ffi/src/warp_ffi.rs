// Phase 4C: PuppetWarp FFI
// Why: Swift 側 `PuppetWarpBridge` から JSON descriptor / params で生成・更新・破棄を行う。

use std::ffi::{c_char, c_void};
use std::sync::Mutex;

use super::conversions::{cstr_to_str, error_to_c_json, rust_str_to_c};
use artia_wgpu::{PuppetWarpDescriptor, PuppetWarpParams, WgpuEngine};
...
macro_rules! lock_engine {
    ($handle:expr, $default:expr) => {
        match $handle.lock() {
            Ok(guard) => guard,
            Err(_) => {
                log::error!("エンジン Mutex ロック失敗 (poisoned) - warp FFI");
                return $default;
            }
        }
    };
...
#[unsafe(no_mangle)]
pub extern "C" fn artia_warp_create(
    engine: *mut c_void,
    ...
    let descriptor: PuppetWarpDescriptor = match serde_json::from_str(json) {
        Ok(d) => d,
        Err(e) => {
            log::error!("artia_warp_create: JSON parse 失敗: {}", e);
            return 0;
        }
    };
    let handle = unsafe { &*(engine as *const EngineHandle) };
    let mut eng = lock_engine!(handle, 0);
    eng.add_puppet_warp(descriptor)
}

/// PuppetWarp に handle を再適用する。
#[unsafe(no_mangle)]
pub extern "C" fn artia_warp_update(
    engine: *mut c_void,
    id: u32,
    ...
    let params: PuppetWarpParams = match serde_json::from_str(json) {
        Ok(p) => p,
        Err(e) => return error_to_c_json(&format!("JSON parse 失敗: {}", e)),
    };
    let handle = unsafe { &*(engine as *const EngineHandle) };
    let mut eng = lock_engine!(handle, error_to_c_json("Mutex poisoned"));
    if eng.update_puppet_warp(id, params) {
        std::ptr::null_mut()
    } else {
        error_to_c_json(&format!("PuppetWarp id={} が見つかりません", id))
    }
}

/// PuppetWarp を破棄する。
#[unsafe(no_mangle)]
pub extern "C" fn artia_warp_destroy(engine: *mut c_void, id: u32) -> u32 {
    if engine.is_null() {
        return 0;
    }
    let handle = unsafe { &*(engine as *const EngineHandle) };
    let mut eng = lock_engine!(handle, 0);
    if eng.remove_puppet_warp(id) { 1 } else { 0 }
}

/// 現在登録されている PuppetWarp 数 (テスト/メトリクス用)。
#[unsafe(no_mangle)]
pub extern "C" fn artia_warp_count(engine: *mut c_void) -> u32 {
    if engine.is_null() {
        return 0;
    }
    let handle = unsafe { &*(engine as *const EngineHandle) };
    let eng = lock_engine!(handle, 0);
    eng.puppet_warp_count() as u32
}

/// JSON ラウンドトリップ確認用。
#[unsafe(no_mangle)]
pub extern "C" fn artia_warp_validate_descriptor(
    descriptor_json: *const c_char,
) -> *mut c_char {
